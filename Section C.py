...
def simulation(interval, x=0.2, q3=False):
    # for the graph
    equity_totals = []
    years = []
    ...
    if not q3:
        ...
        plt.xlabel('Year')
        plt.ylabel('Equity (£)')
        plt.title(f'Accumulated Equity When Buying Every {interval} Years [1974-2014]')
        plt.show()  
...
''' Q3 '''
def q3():
    # create evenly spaced array of xs to try in the simulation
    xs = np.linspace(0.1, 1.0, 91)
    plot_xs = np.linspace(1, 100, 91)
    final_equities = []

    for x in xs:
        final_equity = simulation(5, x, True)
        final_equities.append(final_equity)

    plt.figure(figsize=(10,5))
    plt.plot(plot_xs, final_equities, label='Equity in 2014(£)')
    plt.xlabel('x (%)')
    plt.ylabel('Equity (£)')
    plt.title('Final Equity Using Different [5 Year Intervals]')
    plt.show()
